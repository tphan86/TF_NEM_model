function ktr = rate_force_redev_NEM_1(para_fit,u2,z2,pCa)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Parameter set
    pCa_50 = para_fit(1);
    k0_BC = para_fit(2);
    ...
    for i = 1:m
        for j = 1:n
            neighbor_interaction_para = [alpha,alpha_bar,beta,beta_bar,u1,u2(i),z1,z2(j),v,w];
            p = [ref_para,Ca_Ca50,neighbor_interaction_para,NEM];
            p_resid = [ref_para_resid,Ca_Ca50,neighbor_interaction_para,NEM];
            ...
            F_ss = NEM - y(end,3) + y(end,4);
            Yzero = z(end,:);
            tspan1 = [0,20];
            F_resid = NEM - z(end,3) + z(end,4);
            threshold = F_resid + 0.5*(F_ss - F_resid);
            opt = odeset('Events',@(t,u)  force_redevelop_NEM(t,u,threshold,NEM));
            [t,~] = ode15s(@(t,x) func(t,x,p),tspan1,Yzero,opt);
            ktr(i,j) = - log(1/2)/t(end);
        end
    end
end